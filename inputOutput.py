    ...
    while tmpInput != "0":
        ...
        if roomInput:
            if tmpInput == "1":
                ...
                dorm.rooms.remove(roomInput)
        else:
            if tmpInput == "1":
                dorm.rooms.append(Room(dorm, intInput))
            else:
                print(f"Nie ma takiego pokoju.\n"
                      f"Pokoje w Domu Studenckim {dorm.name()} mają numery od {dorm.rooms[0].number()} do {dorm.rooms[-1].number()}")
